...
func (g *gitHubOutputManager) Put(r ValidationResult) error {
	...
	if !r.ValidatedAgainstSchema {
		g.logger.Printf("::warning file=%s::%s (%s) was not validated against a schema", r.FileName, r.VersionKind(), r.QualifiedName())
		return nil
	}
	...
	for _, e := range r.Errors {
		...
		g.logger.Printf("::error file=%s::%s", r.FileName, e.String())
	}
